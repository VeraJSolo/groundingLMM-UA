...
def parse_args():
    ...
    parser.add_argument("--prompt", type=str, default="Could you please give me a detailed description of the image? Please respond with interleaved segmentation masks for the corresponding parts of the answer.",
...
if __name__ == "__main__":
	...
	# MANUAL ADD TO SEE IMAGE
	# Load original image
	img = cv2.imread(image_path)
	img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)

	# Save overlay if masks exist
	if pred_masks is not None and len(pred_masks) > 0:
    		try:
		        ...
		        overlay_path = f"{args.output_dir}/{image_id[:-4]}_overlay.png"
       			plt.savefig(overlay_path, bbox_inches='tight')
        		plt.close()
    		except Exception as e:
        		print(f"Mask visualization failed for {image_id}: {e}")
	else:
    		print(f"No masks for {image_id}")
	# END MANUAL ADD
